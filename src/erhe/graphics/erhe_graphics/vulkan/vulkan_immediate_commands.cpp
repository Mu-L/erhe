...
void Vulkan_immediate_commands::purge()
{
    ...
    for (uint32_t i = 0; i != buffer_count; i++) {
        ...
        if (wait_result == VK_SUCCESS) {
            ...
            if (result != VK_SUCCESS) {
                m_erhe_device.device_message(Message_severity::error,fmt::format("vkResetCommandBuffer() failed with {} {}", static_cast<int32_t>(result), c_str(result)));
            }

            result = vkResetFences(m_device, 1, &buf.m_fence);
            if (result != VK_SUCCESS) {
                m_erhe_device.device_message(Message_severity::error,fmt::format("vkResetFences() failed with {} {}", static_cast<int32_t>(result), c_str(result)));
            }
            ...
            if (wait_result != VK_TIMEOUT) {
                m_erhe_device.device_message(Message_severity::error,fmt::format("vkWaitForFences() failed with {} {}", static_cast<int32_t>(wait_result), c_str(wait_result)));
            }
        }
    }
}
...
auto Vulkan_immediate_commands::acquire() -> const Vulkan_immediate_commands::Command_buffer_wrapper&
{
    ...
    if (current == nullptr) {
        m_erhe_device.device_message(Message_severity::error,"No available command buffers");
    ...
    if (result != VK_SUCCESS) {
        m_erhe_device.device_message(Message_severity::error,fmt::format("vkBeginCommandBuffer() failed with {} {}", static_cast<int32_t>(result), c_str(result)));
    }
...
void Vulkan_immediate_commands::wait(const Submit_handle handle)
{
    ...
    if (m_buffers[handle.m_buffer_index].m_is_encoding) {
        ...
        m_erhe_device.device_message(Message_severity::error,"waiting for a buffer which has not been submitted (still encoding)");
        return;
    }

    VkResult result = vkWaitForFences(m_device, 1, &m_buffers[handle.m_buffer_index].m_fence, VK_TRUE, UINT64_MAX);
    if (result != VK_SUCCESS) {
        m_erhe_device.device_message(Message_severity::error,fmt::format("vkWaitForFences() failed with {} {}", static_cast<int32_t>(result), c_str(result)));
    }
...
auto Vulkan_immediate_commands::submit(const Command_buffer_wrapper& wrapper) -> Submit_handle
{
    ...
    if (result != VK_SUCCESS) {
        m_erhe_device.device_message(Message_severity::error,fmt::format("vkEndCommandBuffer() failed with {} {}", static_cast<int32_t>(result), c_str(result)));
    }
    ...
    if (m_has_EXT_device_fault && (submit_result == VK_ERROR_DEVICE_LOST)) {
        ...
        if (result != VK_SUCCESS) {
            m_erhe_device.device_message(Message_severity::error,fmt::format("vkGetDeviceFaultInfoEXT() failed with {} {}", static_cast<int32_t>(result), c_str(result)));
        }
