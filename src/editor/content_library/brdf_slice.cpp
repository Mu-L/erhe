...
void Brdf_slice_rendergraph_node::execute_rendergraph_node(erhe::graphics::Command_buffer& command_buffer)
{
    ...
    // Resolved against the set this pass binds, below, and only after
    // sync_library() has given the material its slot.
    ...
    light_projections.brdf_material_slot = m_material_set.get_slot(m_material.get()).value_or(0u);
