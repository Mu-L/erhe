...
vec4 sample_texture(uvec2 texture_handle, vec2 texcoord) {
    ...
    return texture(erhe_texture_heap[texture_handle.x], texcoord);
...
#elif defined(ERHE_TEXTURE_HEAP_VULKAN_DESCRIPTOR_INDEXING)
    return texture(erhe_texture_heap[texture_handle.x], transformed_texcoord);
#elif defined(ERHE_TEXTURE_HEAP_METAL_ARGUMENT_BUFFER)
...
#elif defined(ERHE_TEXTURE_HEAP_VULKAN_DESCRIPTOR_INDEXING)
    return texture(erhe_texture_heap[texture_handle.x], texcoord, lod_bias);
#elif defined(ERHE_TEXTURE_HEAP_METAL_ARGUMENT_BUFFER)
...
#elif defined(ERHE_TEXTURE_HEAP_VULKAN_DESCRIPTOR_INDEXING)
    return textureSize(erhe_texture_heap[texture_handle.x], 0);
#elif defined(ERHE_TEXTURE_HEAP_METAL_ARGUMENT_BUFFER)
