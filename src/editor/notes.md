...
- **Property tools**: `get_item_properties` / `set_item_property` - the generic erhe::property view of any item (registered properties with value, source, default, enum labels, expression text and error; undoable writes of a value or an `expression` formula through `Property_set_operation`; the item reports `sealed` and `style`), `set_item_style` / `clear_item_style` (the style layer, D25, through `Style_set_operation`). See doc/property-system-plan.md D13, D22, D25.
