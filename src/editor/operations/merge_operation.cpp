...
void Merge_operation::execute(App_context& context)
{
    ...
    // Announce the survivor's geometry swap (eager housekeeping for the
    // component-selection store; any entry for the old geometry goes dormant).
...
void Merge_operation::undo(App_context& context)
{
    ...
    // original survivor geometry, so any dormant entry for it becomes live again
    // automatically via is_live().
    context.app_message_bus->mesh_geometry_changed.send_message(
        Mesh_geometry_changed_message{.mesh = m_sources.front().mesh}
    );
