...
layout(std140, binding = 3) uniform view_block {
    DebugViewCamera cameras[view_count];
...
`view_count` is a constructor parameter on
`Debug_renderer` / `Debug_renderer_program_interface` and threads
through from `editor.cpp` as `xr_view_count` (taken from the
...
public:
    ...
    // views.size() must equal view_count from construction.
    void begin_frame(
        erhe::math::Viewport                      viewport,
...
`view_count`, so single-view callers do not over-allocate.
...
- **Triangle SSBO sizing**: multiview needs `view_count * 6 *
  ...
  not `view_count`, so single-view callers do not over-allocate.
