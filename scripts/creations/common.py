...
class Creation:
    ...
    def settle(self, deadline_s=600.0, extra_sleep=0.0):
        ...
        pending_scene_commits + asset_loads == 0. Polls at 0.1 s - typical
        geometry ops finish inside one or two cycles, and settle sleeps (not
        MCP latency) dominate a build's wall time."""
        deadline = time.time() + deadline_s
        while time.time() < deadline:
